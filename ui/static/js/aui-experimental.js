    ...
    */
    ...
    _.extend(Model.prototype, Events, {
      ...
      save: function(key, val, options) {
        ...
        // Do not persist invalid c_models.
        if (!this._validate(attrs, options)) return false;
    ...
    // If c_models tend to represent a single row of data, a Backbone Collection is
    // more analagous to a table full of data ... or a small slice or page of that
    // table, or a collection of rows that belong together for a particular reason
    // -- all of the messages in this particular folder, all of the documents
    // belonging to this particular author, and so on. Collections maintain
    // indexes of their c_models, both in order, and for lookup by `id`.
  
    // Create a new **Collection**, perhaps to contain a specific type of `model`.
    // If a `comparator` is specified, the Collection will maintain
    // its c_models in sort order, as they're added and removed.
    ...
    _.extend(Collection.prototype, Events, {
      ...
      // Add a model, or list of c_models to the set.
      add: function(models, options) {
        return this.set(models, _.defaults(options || {}, addOptions));
      },
  
      // Remove a model, or a list of c_models from the set.
      ...
      // Update a collection by `set`-ing a new list of c_models, adding new ones,
      // removing c_models that are no longer present, and merging c_models that
      ...
      set: function(models, options) {
        ...
        // Turn bare objects into model references, and prevent invalid c_models
        // from being added.
        for (i = 0, l = models.length; i < l; i++) {
          ...
          // This is a new model, push it to the `toAdd` list.
            ...
            // Listen to added c_models' events, and index c_models for lookup by
            // `id` and by `cid`.
        ...
        // Remove nonexistent c_models if appropriate.
        if (options.remove) {
        ...
        // See if sorting is needed, update `length` and splice in new c_models.
        if (toAdd.length) {
      ...
      // you can reset the entire set with a new list of c_models, without firing
      ...
      // Slice out a sub-array of c_models from the collection.
      ...
      // Return c_models with matching attributes. Useful for simple cases of
      // `filter`.
      ...
      // Fetch the default set of c_models for this collection, resetting the
      ...
      // **parse** converts a response into a list of c_models to be added to the
      // collection. The default implementation is just to pass it through.
      parse: function(resp, options) {
        return resp;
      },
  
      // Create a new collection with an identical list of c_models as this one.
      clone: function() {
      ...
      // Sets need to update their indexes when c_models change ids. All other
    ...
    // Mix in each Underscore method as a proxy to `Collection#c_models`.
    ...
    // react to specific changes in the state of your c_models.
    ...
    // c_models to the server. You will be passed the type of request, and the
    // model in question. By default, makes a RESTful Ajax request
    // to the model's `url()`. Some possible customizations could be:
    //
    // * Use `setTimeout` to batch rapid-fire updates into a single request.
    // * Send up the c_models as XML instead of JSON.
    // * Persist c_models via WebSockets instead of Ajax.
    //
...
(typeof window === 'undefined' ? global : window).__4f7eb6cf60845d0cd685bf4d782df3ea = (function () {
  ...
  var ProgressiveDataSet = _backbone2.default.Collection.extend({
       ...
       *
       * @param query the value to check in the cache
       * @return {Object[]} an array of values representing the IDs of the c_models the response for this query contained.
       */
      ...
      _createCollection: function _createCollection() {
          ...
          return new RowsAwareCollection([], {
              comparator: function comparator(row) {
                  // sort c_models in collection based on dom ordering
                  var index;
...
(typeof window === 'undefined' ? global : window).__67239d8727b67748b2105d3faf199585 = (function () {
  ...
  function initialiseProgressiveDataSet(element) {
      ...
      // must manually convert all data list options into c_models.
      //
...
(typeof window === 'undefined' ? global : window).__b7e5361a4558fb1fdb441a3762d29b73 = (function () {
  ...
  var TruncatingProgressiveDataSet = _progressiveDataSet2.default.extend({
       ...
       * @return {Object[]} an array of values representing the IDs of the c_models provided by the response for the given query.
